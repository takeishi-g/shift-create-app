...
const MOCK_STAFF: StaffProfile[] = [
  ...
  { id: 'st-4', name: '堀 奈々美',  qualification: '正看護師', role: '一般', work_hours_type: 'AM', experience_years: 6,  max_hours_per_month: 160, max_night_shifts: 6, is_active: true, created_at: '', updated_at: '' },
  { id: 'st-5', name: '伊藤 健二',  qualification: '准看護師', role: '一般', work_hours_type: 'PM', experience_years: 4,  max_hours_per_month: 120, max_night_shifts: 4, is_active: true, created_at: '', updated_at: '' },
]
  ...
  const dailyCounts = useMemo(() => {
    return days.map((_, i) => {
      ...
      rows.forEach(({ staff, shifts }) => {
        ...
        // 日勤: work_hours_type で分類
        if (code === '日') {
          if (staff.work_hours_type === 'AM') am++
          else pm++
        }
      })
