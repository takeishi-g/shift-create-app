...
原因2（森さん7/1夜勤希望×月跨ぎ強制公休）は**データ不備ではなく仕様の未実装**だった。
CONSTRAINTS.md 4章「月跨ぎ夜勤」には元々「連続夜勤の場合は明→夜→明→公→公」と
記載されていたが、ソルバーは前月末が夜→明のとき翌月1日を無条件に公休固定していた。

対応（実装済み）:
- `buildPrevMonthInfo` に `nightOrOffDays` を追加。前月末が夜→明で終わった翌日は
  「公休 or 夜勤」を許可（日勤のみ禁止）。夜勤を選んだ場合は明け後に2日公休を強制
  （`cross_double_night_off` 制約）。s1パス・メインパス両方
- 前月末が2連続夜勤の末尾（…明→夜）の場合は従来どおり翌月1日=明・2日=公に加え、
  3日も公休に固定（2日公休の完全化）
- プリフライト: nightOrOff日の夜勤希望は矛盾扱いしない。日勤希望のみ矛盾として検出
- 制限: tail は前月末2日分のみのため「夜→明」がすでに2連続夜勤の末尾かは判定不能

検証（本番同等入力）:
- 現状データ: 検出される矛盾は「山下さん×藤田さん 7/2 夜勤希望×ペア制約」のみ
- どちらか一方の7/2希望を除けば CSP success。森さんは 7/1夜→7/2明→7/3希休→7/4希休
  （夜明夜明休休の月跨ぎ）が反映される

残る確認事項: 山下さん・藤田さんの7/2夜勤希望はDB上どちらも「7/2開始の夜勤」（同じ夜に勤務）。
「夜と明」の関係にするには一方の希望日を7/1に変更する必要がある（運用判断待ち）。

### データ整理（2026-06-11 実施済み）

- ユーザー判断により、藤田さんの 7/2 夜勤シフト希望（leave_requests id:
  7997ca79-fb4d-43f7-8a06-27259eebf719）を削除した。
- これで2026-07のデータ上の矛盾はすべて解消。本番同等入力での検証により
  CSP success（山下さん7/2夜・森さん7/1夜→明→希休→希休が反映）を確認済み。
- 残タスクはすべて完了。未着手の既知バグ（constraints ページ保存処理の
  エラー未チェック、フォールバックの夜勤候補再チェック）は別対応とする。
